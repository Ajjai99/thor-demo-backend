...
variable "api_gateway_certificate_key" {
  type        = string
  description = "Which entry in the flattened hosted_zones certificates (key format \"<zone_key>/<cert_key>\", e.g. \"thor/api_gateway\") the API Gateway custom domain + cert come from. \"\" (default) leaves the API reachable only via its default execute-api URL, no custom domain created."
  default     = ""
}
